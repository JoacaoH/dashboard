...
export function Frequency({ userEmail }: { userEmail: string }) {
  ...
  useEffect(() => {
    ...
    async function loadDeliveryData() {
      try {
        const res = await axios.get('http://localhost:8000/api/stats/frequency', {
  ...
  ? Math.max(...deliveryData.map(item => item.quantidade))
  : 100; 


  return (
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Encontros</h3>
        <p className="text-sm text-gray-600 mb-4">Percentual de presença por encontro</p>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={deliveryData}>
              ...
              <YAxis 
                ...
                />
              <Tooltip
              formatter={(value: number) => [`${value}`, 'Presentes']}
              />
